...
resource "aws_lb_target_group" "st1_ex_docker_sub_tg" {
    name = "st1-ex-docker-sub-tg"
    port = 8081
    protocol = "HTTP"
    vpc_id = data.aws_vpc.st1_ex_vpc.id
    ...
    tags = {name = "st1-ex-docker-sub-tg"}

      
}

resource "aws_lb_target_group_attachment" "st1_ex_docker_sub_tg" {
  #어느 대상 그룹에 등록할 것인가
  target_group_arn = aws_lb_target_group.st1_ex_docker_sub_tg.arn
  target_id = aws_instance.st1_ex_alb_instance[0].id
...
resource "aws_lb_listener_rule" "sub_path_sub_rule" {
...
#5. FastAPI 대상 그룹
resource "aws_lb_target_group" "st1_ex_fastapi_tg" {
    name = "st1-ex-fastapi-tg"
    port = 8000
    protocol = "HTTP"
    vpc_id = data.aws_vpc.st1_ex_vpc.id
    ...
    tags = { name = "st1-ex-fastapi-tg" }
}

resource "aws_lb_target_group_attachment" "st1_ex_fastapi_tg_attachment" {
  target_group_arn = aws_lb_target_group.st1_ex_fastapi_tg.arn
  target_id = aws_instance.st1_ex_alb_instance[0].id
  port = 8000
}
